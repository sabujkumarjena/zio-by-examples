package com.allevite.zioByExamples.section2

import zio.*
import com.allevite.utils.*
...
object Fibers extends ZIOAppDefault {
  ...
  val chainedFibers = for {
    ...
    fiber2 <- ZIO.succeed("Scala Rocks!!").debugThread.fork
  ...
  def wordCountParallel(n: Int): UIO[Int] = {
    val effects: Seq[ZIO[Any, Nothing, Int]] = (1 to n)
      .map(i => s"src/main/resources/test_$i.txt") // paths
  ...
  def run = {
    //sameThreadIO
   // differentThreadIO
  // ZIO.succeed(generateRandomFile("src/main/resources/test.txt"))
  //ZIO.succeed((1 to 10).map(i => generateRandomFile(s"src/main/resources/test_$i.txt")))
  ZIO.succeed(countWords("src/main/resources/test.txt"))
  //ZIO.succeed(wordCountParallel(10))
  }
}
