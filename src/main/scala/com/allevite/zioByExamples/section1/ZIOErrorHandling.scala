...
object ZIOErrorHandling extends ZIOAppDefault {
  ...
  // use attempt if you'are ever unsure whether your code might throw error '
  ...
  // option -> ZIO
  val anOption: ZIO[Any, Option[Nothing], Int] = ZIO.fromOption(Some(46))
  ...
  val endpointCallWithDefects: ZIO[Any, Nothing, String] =
    callHTTPEndpoint("allevite.com").orDie // all errors are now defects

      // refining the error channel
  def callHTTPEndpointWideError(url: String): ZIO[Any, IOException, String] =
  ...
  val callApi: ZIO[Any, IndexError, String] =
    ZIO.succeed("page: <html></html>")
   ...
   */
  val combined_v2: ZIO[Any, IndexError | DbError, (String, Int)] = for {
    page <- callApi
    rowsAffected <- queryDb
