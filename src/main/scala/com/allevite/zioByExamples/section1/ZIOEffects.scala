...
object ZIOEffects {
  // success
  val mol: ZIO[Any, Nothing, Int] = ZIO.succeed(45)
  // failure
  val aFailure: ZIO[Any, String, Nothing] = ZIO.fail("Something went wrong")
  // suspension/delay
  val aSuspendedZIO: ZIO[Any, Throwable, Int] = ZIO.suspend(mol)

  // map + flatMap
  val improvedMOL = mol.map(_ * 2)
  val printingMOL = mol.flatMap(mol => ZIO.succeed(println(mol)))
  // for comprehensions
  ...
  val tupledZIO = mol.zip(anotherMOL)
  val combinedZIO = mol.zipWith(anotherMOL)(_ * _)
